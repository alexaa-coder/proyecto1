...
def justificar_texto(texto: str) -> str:
    ...
    Usa <div> con sintaxis compatible y asegura interoperabilidad con Markdown.
    """
    debug_log("Aplicando justificación de texto...")
    
    # Separar por bloques (línea vacía como delimitador)
    bloques = re.split(r"(\n{2,})", texto)
    resultado = []
    
    # Marcadores de bloques que NO deben justificarse
    # Incluimos los escapes de HTML (&lt;, &#123;)
    RE_STRUCTURAL = re.compile(
        r"^(#|\-|\*|\d+\.|\||```|<img|<p|<div|<br|<hr|&lt;|&#123;|--|id:|title:|sidebar_label:)",
        re.IGNORECASE
    )

    for bloque in bloques:
        content = bloque.strip()
        # Si es un delimitador de línea vacía o bloque vacío, mantener
        if not content:
            resultado.append(bloque)
            continue
            
        # Si es un bloque estructural o extremadamente corto, no justificar
        lines = content.split('\n')
        if any(RE_STRUCTURAL.match(line.strip()) for line in lines) or len(content) < 50:
            resultado.append(bloque)
        else:
            # Párrafo normal -> Justificar usando <div>
            # Importante: MDX requiere líneas en blanco para parsear Markdown dentro de etiquetas JSX
            justificado = f'<div style={{{{textAlign: "justify"}}}}>\n\n{content}\n\n</div>'
            resultado.append(justificado)
