...
|Esquema de Firm|Estándar LoaIKEv2 cl|ngitud deave (bits)|Fortaleza(bits)|CAT|
|---|---|---|---|---|
|** XTENSIÓN DEL MODEL**|**  O**||||
|demás de las característicancionalidad incorporando n|s principales, el Modelo uevas características, con|de Diaman   el objetivo|te puede    de lograr|ampl     una|
|efinición de los eventos y, en|    su consecuencia, la condu|     cta de los ad|       versarios.| |
|sí, podemos incluir dos nu_dversario_ con su_Víctima_ y|evas meta-características: la**_Tecnológica_**, que vin|a**_Socio-Po_** cula a la_I_|**_lítica_**, que_nfraestruc_|vinc_tura_ c|
|_apacidad_, tal y como se repr|esenta en la figura siguiente|.|||
||||||
...
|---|---|---|---|---|---|---|
||ecesidades socio-políctima de satisfacer|ticas, aspiraciones y motivos  tales necesidades), y la car|del advers    acterística|ario y la d    Tecnológ|isponibilidad    ica, que enla|isponibilidad    ica, que enla|
||apacidad usada porcoger datos sobre la|el adversario con la Infraest  tecnología que posibilita la re|ructura sob    lación ant|re la que     rior).|se despliega|se despliega|
||A CARACTERÍSTI| CA SOCIO-POLÍTICA|||||
||||||||
|**A**_r_|**xioma 6**: _Siempre_|_ existe una relación entre el_|_   Adversari_|_   o y su Ví_|_     ctima, aunq_|_     ue tal_|
|**A**_r_|_lación pueda ser dis_|_  tante, fugaz o indirecta._|||||
|**A**_r_|||||||
|**A**_r_|l vínculo_Adversario_ las necesidades y a|_-Víctima_ se basa en una rela  spiraciones socio-políticas de|ción produ   l adversari|ctor-cons    o (generar|umidor, suste     ingresos, lo|umidor, suste     ingresos, lo|
|**A**_r_|ceptación en una det|  erminada comunidad, aument|   ar los bene|    ficios emp|    resariales, o|    resariales, o|
|**A**_r_|formación valiosa,onsumidor como l|tc.). Esta relación comprend a capacidad de la víctim|e tanto las   a-producto|necesida   r para s|es del adver    atisfacer aq|es del adver    atisfacer aq|
...
|no revelar actividad relacionada con otraracterística de arranque. Seguidamente, se|as caracteexaminan|rísticas conelos posibles|ctadas o coorígenes del|n la pranálisi|
|---|---|---|---|---|
|**La****_Víctima_ como origen del análisis**:|el análisis|de datos de|una poten|cial víc|
|vela otros elementos relacionados, coneapacidades del adversario y la infraestructu|tados a t   ra sobre la|ravés de las    que se despl|aristas del     egan27.|evento|
|**La****_Capacidad_ como origen del análi**|**  is**: en es| te tipo de a|   nálisis se|explota|
|aracterísticas de la capacidad para des|   cubrir otr|   os elemento|    s relaciona|    dos en|
...
|.11|SEL|Col3|LOS DE TIE|Col5|MP|O [MP.INFO.4]|Col8|Col9|Col10|
|---|---|---|---|---|---|---|---|---|---|
||9|9|Hipótesis|Hipótesis||El Adversario1 verifica lbuscando aquellos queS.A., descubriendo la diInvestigaciones.|El Adversario1 verifica lbuscando aquellos queS.A., descubriendo la diInvestigaciones.|El Adversario1 verifica lbuscando aquellos queS.A., descubriendo la diInvestigaciones.|a lista de contactos de correo de AR1,  pudieran ser de Investigaciones Frutícolas,  rección de correo electrónico de su Director de|
||10|10|Real|Real||El Director de Investigaun correo spear-phisinincluyendo un adjunto|El Director de Investigaun correo spear-phisinincluyendo un adjunto|El Director de Investigaun correo spear-phisinincluyendo un adjunto|ciones de Investigaciones Frutícolas, S.A. recibe g de AR1 de Productos Frutícolas, S.A.,  con el mismo código dañino anterior.|
||11|11|Real|Real||Un Adversario desconovulnerables, incluyend|Un Adversario desconovulnerables, incluyend|Un Adversario desconovulnerables, incluyend|cido explora la web en busca de web servers o a Víctima 3.|
||12|12|Real|Real||Un exploit para una vul|Un exploit para una vul|Un exploit para una vul|nerabilidad se remite a la Víctima 3.|
||13|13|Real|Real||El servidor atacado deAdversario.|El servidor atacado deAdversario.|El servidor atacado deAdversario.|Víctima 3 establece una conexión remota con el|
||14|14|Real|Real||El Adversario usa la conde en el directorio priv|El Adversario usa la conde en el directorio priv|El Adversario usa la conde en el directorio priv|exión remota para descargar los documentos  ado de Víctima 3.|
||La tabla siguiente|La tabla siguiente|La tabla siguiente|La tabla siguiente|describe los arcos de la f|describe los arcos de la f|describe los arcos de la f|describe los arcos de la f|igura 7.|
|**Ar**|**co**|**Confianza**|**Confianza**|**A**|**ND/OR**|**ND/OR**|**Hipótesis/Real**||**Proporciona**|
||A|Baja|Baja||AND|AND|Real||Proporciona el dominio para ProductosFrutícolas, S.A. (www.dehigosabrevas.com)|
||B|Alta|Alta||AND|AND|Real|  |Proporciona los objetivos del spear-phising:direcciones de correo electrónico de losadministradores de red dewww.dehigosabrevas.com|
||C|Alta|Alta||AND|AND|Real||Ninguno|
||D|Alta|Alta||OR|OR|Real||Ninguno|
||D|Alta|Alta|||||||
||E|Alta|Alta||OR|OR|Real||Ninguno|
||F|Alta|Alta||AND|AND|Real||Ninguno|
||G|Alta|Alta||AND|AND|Real||Ninguno|
||H|Media|Media||AND|AND|Real||Proporciona acceso proxy desde víctimaanterior al motor de búsqueda.|
||I|Baja|Baja||AND|AND|Hipótesis||Acceso a la lista de contactos.|
||J|Alta|Alta||AND|AND|Real||Identificación de la organización de la víctima.|
||K|Baja|Baja||AND|AND|Hipótesis||Dirección de correo electrónico, nombre eidentificación del rol de la víctima.|
|L|L|Alta|Alta|AND|AND|AND|Real|Correo electrónico de spear-phisingtroyanizado.|Correo electrónico de spear-phisingtroyanizado.|
|M|M|Alta|Alta|AND|AND|AND|Real|Proporciona los resultados satisfactorios delanálisis que identifica el servidor web de lavíctima como vulnerable al ataque.|Proporciona los resultados satisfactorios delanálisis que identifica el servidor web de lavíctima como vulnerable al ataque.|
|N|N|Alta|Alta|AND|AND|AND|Real|Ninguno|Ninguno|
|O|O|Alta|Alta|AND|AND|AND|Real|Proporciona el establecimiento de la conexiónremota.|Proporciona el establecimiento de la conexiónremota.|
...
Los arcos están etiquetados con una tupla del tipo: 
