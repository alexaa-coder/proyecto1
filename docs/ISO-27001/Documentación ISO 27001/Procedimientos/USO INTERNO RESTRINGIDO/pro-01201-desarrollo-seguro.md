...
|HISTORIAL DEL DOCUMENTO|Col2|Col3|Col4|
|---|---|---|---|
|Asimismo, tNacional de|Referencia:https://owasp.org/www-project-code Seguridad, referenciados en elAnexo II. Checklist|-review-guid|/      cidos por el Esq   seguridad.|
|Asimismo, tNacional de|Referencia:https://owasp.org/www-project-code Seguridad, referenciados en elAnexo II. Checklist|uías estable  requisitos de|uías estable  requisitos de|
|Asimismo, tNacional de|Referencia:https://owasp.org/www-project-code Seguridad, referenciados en elAnexo II. Checklist|||
|** Aspec**|** tos de seguridad en el diseño integral del s**|**     istema**||


software, se propone revisar los puntos técnicos recogidos en el proyecto OWASP Application

|Security Verification Standard (ASVS), sobre los documentos de diseño de cada proyec|Col2|Col3|Col4|
|---|---|---|---|
|Security Verification Standard (ASVS), sobre los documentos de diseño de cada proyedesarrollo y posteriormente también sobre su implementación. A continuación, su det|Security Verification Standard (ASVS), sobre los documentos de diseño de cada proyedesarrollo y posteriormente también sobre su implementación. A continuación, su det|Security Verification Standard (ASVS), sobre los documentos de diseño de cada proyedesarrollo y posteriormente también sobre su implementación. A continuación, su det|Security Verification Standard (ASVS), sobre los documentos de diseño de cada proyedesarrollo y posteriormente también sobre su implementación. A continuación, su det|
|eferencia:• **Che**|**cklist “OWASP Applicati**|** on Security Verification St**|**   andard (ASVS)”**|
|El Prun Caplic|oyecto del Estándar de V hecklist, proporciona unaaciones web y también pro|erificación de Seguridad de  base para probar los contro  porciona a los desarrollado|Aplicaciones de OWASP, q    les técnicos de seguridad    es una lista de requisitos p|
|desa`o` h|rrollo seguro. Referenciasttps://owasp.org/www-proj| :ect-application-security-veri|fication-standard/|
|`o` h• CheRefe|ttps://mas.owasp.org/MAS|VS/    seguridad exigidas por el Checklist requisitos de segu|     Esquema Nacional de Segu   ridad.|
|`o` h• CheRefe|cklist sobre las medidas derenciadas en elAnexo II.|cklist sobre las medidas derenciadas en elAnexo II.|cklist sobre las medidas derenciadas en elAnexo II.|
|**7. DE**|** SARROLLO SEGURO**|||
...
|Col1|Col2|Col3|
|---|---|---|
|funcionalidadfacilitar el ucredencialescualquier se**_Evidencias_**| debe estar siempre fácilme  so de esta. Así mismo esta, por ejemplo, invalidando e sión anterior al cambio.**_  solicitar:_**|    nte visible y accesible por los usuarios, para concien        funcionalidad se llamará en el proceso de cam|
|funcionalidadfacilitar el ucredencialescualquier se**_Evidencias_**| debe estar siempre fácilme  so de esta. Así mismo esta, por ejemplo, invalidando e sión anterior al cambio.**_  solicitar:_**|    n ese momento del cambio a las nuevas credenc|
|funcionalidadfacilitar el ucredencialescualquier se**_Evidencias_**| debe estar siempre fácilme  so de esta. Así mismo esta, por ejemplo, invalidando e sión anterior al cambio.**_  solicitar:_**||
|`o` Captdurac`o` Captsesió`o` Captduran**7.2.7**Para evitar u| ura de pantalla de la configión de las sesiones.  |uración del sistema que muestre cómo se establ|
|`o` Captdurac`o` Captsesió`o` Captduran**7.2.7**Para evitar u| ura de pantalla de la configión de las sesiones.  ||
|`o` Captdurac`o` Captsesió`o` Captduran**7.2.7**Para evitar u|ura de pantalla del código qn.|ue muestra cómo se generan y gestionan los toke|
|`o` Captdurac`o` Captsesió`o` Captduran**7.2.7**Para evitar u|ura de pantalla del código ote el proceso de cambio de|documento que muestre cómo se manejan las ses      redenciales.|
|`o` Captdurac`o` Captsesió`o` Captduran**7.2.7**Para evitar u|** Seguridad de base de d**  n ataque de tipo inyección S||
|`o` Captdurac`o` Captsesió`o` Captduran**7.2.7**Para evitar u|** Seguridad de base de d**  n ataque de tipo inyección S|**     atos**       QL, aparte de establecer un control de validación de|
|de entrada cparametrizadA continuacipara conformcomandos mconsulta SQpublic void Foo&#123;     string sensiti    context.Data    context.Quer     context.Datathe Formattable    string query    context.Datawon't be conver&#125;  public void Bar(|  omo se especifica en el prias mediante objetos que pro|       mer punto, siempre usaremos consultas a base de    tejan de este tipo de ataque.|
|de entrada cparametrizadA continuacipara conformcomandos mconsulta SQpublic void Foo&#123;     string sensiti    context.Data    context.Quer     context.Datathe Formattable    string query    context.Datawon't be conver&#125;  public void Bar(|ón se muestra un ejemplo de ar la futura consulta SQL a aliciosos al concatenarse n L:  (DbContext context, string query, st|un código vulnerable, el cual concatena cadenas de       base de datos. En este tipo de código se pueden in|
|de entrada cparametrizadA continuacipara conformcomandos mconsulta SQpublic void Foo&#123;     string sensiti    context.Data    context.Quer     context.Datathe Formattable    string query    context.Datawon't be conver&#125;  public void Bar(|ón se muestra un ejemplo de ar la futura consulta SQL a aliciosos al concatenarse n L:  (DbContext context, string query, st|    uestra posible entrada de datos maliciosa al resto|
|de entrada cparametrizadA continuacipara conformcomandos mconsulta SQpublic void Foo&#123;     string sensiti    context.Data    context.Quer     context.Datathe Formattable    string query    context.Datawon't be conver&#125;  public void Bar(|ón se muestra un ejemplo de ar la futura consulta SQL a aliciosos al concatenarse n L:  (DbContext context, string query, st|ring param)|
|de entrada cparametrizadA continuacipara conformcomandos mconsulta SQpublic void Foo&#123;     string sensiti    context.Data    context.Quer     context.Datathe Formattable    string query    context.Datawon't be conver&#125;  public void Bar(|     veQuery = string.Concat(query, par|        am);|
|de entrada cparametrizadA continuacipara conformcomandos mconsulta SQpublic void Foo&#123;     string sensiti    context.Data    context.Quer     context.Datathe Formattable    string query    context.Datawon't be conver&#125;  public void Bar(|base.ExecuteSqlCommand(sensitiv    y().FromSql(sensitiveQuery    base.ExecuteSqlCommand($"SELE|eQuery); // Sensitive    ); // Sensitive|
|de entrada cparametrizadA continuacipara conformcomandos mconsulta SQpublic void Foo&#123;     string sensiti    context.Data    context.Quer     context.Datathe Formattable    string query    context.Datawon't be conver&#125;  public void Bar(|base.ExecuteSqlCommand(sensitiv    y().FromSql(sensitiveQuery    base.ExecuteSqlCommand($"SELE|CT * FROM mytable WHERE mycol=&#123;value&#125;", param); // Sensit|
|de entrada cparametrizadA continuacipara conformcomandos mconsulta SQpublic void Foo&#123;     string sensiti    context.Data    context.Quer     context.Datathe Formattable    string query    context.Datawon't be conver&#125;  public void Bar(|String is evaluated and converted      = $"SELECT * FROM mytable WHE    base.ExecuteSqlCommand(query);  ted to a parametrized query.  SqlConnection connection, string p|o RawSqlString           RE mycol=&#123;param&#125;";     // Sensitive, the FormattableString has already been evaluated,|
|de entrada cparametrizadA continuacipara conformcomandos mconsulta SQpublic void Foo&#123;     string sensiti    context.Data    context.Quer     context.Datathe Formattable    string query    context.Datawon't be conver&#125;  public void Bar(|String is evaluated and converted      = $"SELECT * FROM mytable WHE    base.ExecuteSqlCommand(query);  ted to a parametrized query.  SqlConnection connection, string p||
|de entrada cparametrizadA continuacipara conformcomandos mconsulta SQpublic void Foo&#123;     string sensiti    context.Data    context.Quer     context.Datathe Formattable    string query    context.Datawon't be conver&#125;  public void Bar(|String is evaluated and converted      = $"SELECT * FROM mytable WHE    base.ExecuteSqlCommand(query);  ted to a parametrized query.  SqlConnection connection, string p|aram)|
...
&gt; Table omitted (Table 46): duplicated data detected.
...
|Col1|PROCEDIMIENTO DEGESTIÓN Y NOTIFICACIÓNDE INCIDENTES DE SEGURIDADPROCEDIMIENTO DEDESARROLLO SEGURO|Código: PRO-015.01Código: PRO-012.01|
|---|---|---|
||USO OFICIAL INTERNO_RESTRINGIDO_|Fecha de vigor: 12/03/2025 |
...
|denegación de acceso a servicios ccomprensión, estrés, dolencias físicas|omerciales, miedo, falta deCATEGORÍA DEL SISTEMA|Col3|Col4|
|---|---|---|---|
||**MEDIA**|**ALTA**|**COMPROBACIÓN**|
