# converter.py
# Document Converter for Docusaurus - Enhanced Edition
...
def handle_pdf(file_path: Path, img_root: Path) -> Optional[str]:
    ...
    try:
        ...
        log.error("pymupdf4llm not installed. Skip PDF processing or install it.")
        return None
...
def handle_docx(file_path: Path, img_root: Path) -> Optional[str]:
    ...
    try:
        cmd = ["pandoc", str(file_path), "-t", "markdown", "--extract-media", str(img_dir), "-o", str(output_temp)]
        ...
        if output_temp.exists(): output_temp.unlink()
...
def handle_excel(file_path: Path) -> Optional[str]:
    ...
    try:
        ...
        log.error("pandas or tabulate not installed. Skip Excel processing.")
        return None
...
def filter_deduplicated_files(files: List[Path]) -> List[Path]:
    """Skips PDFs if a corresponding DOCX exists in the same folder."""
    filtered = []
    ...
    for f in files:
        ext = f.suffix.lower()
        key = (f.parent, f.stem)
        if ext == ".pdf" and ".docx" in file_map[key]:
            log.info(f"Skipping redundant PDF (DOCX exists): {f.name}")
            continue
        filtered.append(f)
    return filtered
...
def main():
    ...
    log.info("Check conversion.log for details.")
