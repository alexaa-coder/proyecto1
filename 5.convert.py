...
def arreglar_html_mdx(texto: str) -> str:
    """Convierte etiquetas HTML a JSX."""
...
def aplicar_ocr_a_pdf(pdf_path: Path) -> Tuple[Optional[str], bool]:
    ...
    Returns:
        Tuple[texto_extraido, se_uso_ocr]
    """
...
def handle_pdf(file_path: Path) -> Optional[str]:
    ...
    Detecta automáticamente PDFs escaneados y aplica OCR si es necesario.
    """
...
def handle_png(file_path: Path) -> Optional[str]:
    ...
    Si no hay texto reconocible, inserta como imagen enlazada.
    """
    if not OCR_DISPONIBLE:
...
def handle_zip(file_path: Path, output_base_dir: Path) -> Tuple[int, int]:
    ...
    Returns:
        Tuple[archivos_procesados, archivos_fallidos]
    """
    import zipfile
...
def convertir_archivo(file_path: Path, output_path: Path, verbose: bool = True) -> bool:
    ...
    Manejo robusto de errores para no detener el proceso completo.
    """
    ...
    try:
        ...
        debug_log("⏱  Tiempo: {t:.2f}s", t=t_total)
